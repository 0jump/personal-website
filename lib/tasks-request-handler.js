    ...
    _executeFunc() {
        switch (this.func) {
            ...
            case "getTaskWithChildrenDeepWithPermissionsSharedWithUser":
                ...
                break;
            case "getTaskWithChildrenDeepWithPermissionsSharedWithUserAndElements":
                this._getTaskWithChildrenDeepWithPermissionsSharedWithUserAndElements();
                break;
            ...
            case "addNewElementForTask":
                ...
                break;
            case "deleteElement":
                this._deleteElementAndChangeNextAndPreviousTasks();
                break;
            case "moveElementBefore":
                this._moveElementBefore();
                break;
            default:
...
/*     _setTaskPermissionForTaskAndChildrenDeep(){
    ...
    _getTaskWithChildrenDeepWithPermissionsSharedWithUserAndElements() {
        let taskToGetWithChildrenDeep = this.req.body.task_id;

        dbservices.getTaskWithChildrenDeepWithPermissionsSharedWithUserAndElements(this.authData.user_id, taskToGetWithChildrenDeep, (err_type,err_desc,taskAndChildrenDeepAndElementsArrays) => {
            if(!err_type){
                this.res.status(200).json(taskAndChildrenDeepAndElementsArrays);
            }else{
    ...
    _deleteElementAndChangeNextAndPreviousTasks(){
        let elemToDeleteId = this.req.body.element_id;

        dbservices.deleteElementAndChangeNextAndPreviousTasks(this.authData.user_id, elemToDeleteId, (err_type, err_desc, deletedElementIdAndChangesToImplement) => {
            if(!err_type){
                this.res.status(200).json(deletedElementIdAndChangesToImplement);
            }else{
                if(err_type == "input_error"){
                    this.res.status(400).json({"error": err_desc})
                }else{
    ...
    _moveElementBefore() {
        let elemToMoveId = this.req.body.element_id;
        let newNextSiblingId = this.req.body.next_sibling_id;
        let newTaskId = this.req.body.task_id;

        dbservices.moveElementBeforeAnother(this.authData.user_id, elemToMoveId, newNextSiblingId, newTaskId,(err_type,err_desc, oldAndNewTaskAndChangesToImplement) => {
            if(!err_type){
                this.res.status(200).json(oldAndNewTaskAndChangesToImplement);
            }else{
                if(err_type == "input_error"){
                    this.res.status(400).json({"error": err_desc})
                }else{
