'use strict';
const config = require('./server-configurations.js');
...
let getNotificationsMailOptions = (to, subject, text, html) => {
    ...
    let mailOptions = {
        ...
        subject: 'GA - ' + subject, // Subject line
...
mySmtp.send.notifications.homePageVisit = () => {
    // send mail with defined transport object
    transporter.sendMail(getNotificationsMailOptions('gerard.antoun@yahoo.com', 'Homepage Visit', 'No Plaintext', emTest), (error, info) => {
        if (error) {
            return console.log(error);
        }
        console.log('Message sent: %s', info.messageId);
