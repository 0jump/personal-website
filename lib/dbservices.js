...
db.getTaskChildrenOneLayerAsArray = (pUserId, pTaskId, callback) => {
    if (connection){    
        let query_getTaskChildrenOneLayerAsArray = `SELECT id,title,parent_id,previous_sibling_id,next_sibling_id,creator_user_id,date_created,is_deleted FROM tasks WHERE parent_id = '${pTaskId}' AND creator_user_id = '${pUserId}' AND is_deleted=0;`;
