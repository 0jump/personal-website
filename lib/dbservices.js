...
db.updateTaskNextPreviousParentRelationships = (pUserId, pTaskId, pNewParentId, pNewPreviousSiblingId, pNewNextSiblingId, callback) => {
    
    if (connection){
        let query_updateTaskNextPreviousParentRelationships = `UPDATE tasks SET parent_id=${pNewParentId}, previous_sibling_id ${(pNewPreviousSiblingId == null) ? "= NULL" : "= " + pNewPreviousSiblingId}, next_sibling_id ${(pNewNextSiblingId == null) ? "= NULL" : "= " + pNewNextSiblingId} WHERE id=${pTaskId} AND creator_user_id=${pUserId} AND is_deleted=0;`;
                ...
                if(pTaskObj.parent_id == nextSiblingTaskObj.parent_id){
                    ...
                    db.addNewTaskWithoutTouchingAnyOtherTasks(pUserId,pTaskObj, (err_type, err_desc,newTaskObjId)=> {
                        if(!err_type){
                            ...
                            db.giveOwnerCanGivePermissionsPermission(pUserId, pTaskObj.id, (err_type, err_desc, isPermissionGiven)=>{
                                if(!err_type){
                                    if(isPermissionGiven){
                                        ...
                                        console.log("Check: 100234, something is wrong");
                                    }
                                } else{
...
db.checkIfTaskIsDescendant = (pUserId , pTaskDescendentCandidateId, pTaskParent, callback) => {
...
db.moveTaskBeforeAnother = (pUserId, pTaskToMoveId, pNewNextSiblingId, pNewParentId, callback) => {
    ...
    if(pTaskToMoveId != pNewParentId && pTaskToMoveId != pNewNextSiblingId){
        db.checkIfTaskIsDescendant(pUserId, pNewParentId, pTaskToMoveId, (err_type, err_desc, isDescendant) => {
            if(!err_type){
                if (isDescendant) {
                    callback("input_error", "cannot move task into one of its descendants", {})
                } else {
                    if (pNewParentId != null) {
                        db.getTask(pUserId, pTaskToMoveId, (err_type, err_desc, taskObj) => {
                                ...
                                db._manipulateNextAndPrev.checkIfTaskHasSiblingsAndUpdateSiblingRelations_delete(pUserId, taskObj, db.updateTaskPreviousSibling, db.updateTaskNextSibling, (err_type, err_desc, changesToImplementAfterRemovingTaskFromOldPos) => {
                                    ...
                                    if (!err_type) {
                                        ...
                                        if (taskObj.next_sibling_id != null) {
                                            // Get next sibling task object to get necessary information
                                            db.getTask(pUserId, taskObj.next_sibling_id, (err_type, err_desc, nextSiblingTaskObj) => {
                                                // next sibling does exist
                                                // add task record of pTaskObj to receive its id
                                                if (!err_type) {
                                                    if (taskObj.parent_id == nextSiblingTaskObj.parent_id) {
                                                        ...
                                                        db.updateTaskNextPreviousParentRelationships(pUserId, taskObj.id, taskObj.parent_id, taskObj.previous_sibling_id, taskObj.next_sibling_id, (err_type, err_desc, isUpdated) => {
                                                            if (!err_type) {
                                            ...
                                            // Does not have next sibling check if it has any siblings at all
                                            db.getTask(pUserId, taskObj.parent_id, (err_type, err_desc, parentTaskObj) => {
                                                // add task record of pTaskObj to receive its id
                                                if (!err_type) {
                                                    db.getLastTaskChildOneLayer(pUserId, taskObj.parent_id, (err_type, err_desc, lastChildTaskObj) => {
                                                        if (!err_type) {
                                                            if (lastChildTaskObj != null) {
                                                            ...
                                                            db.updateTaskNextPreviousParentRelationships(pUserId, taskObj.id, taskObj.parent_id, taskObj.previous_sibling_id, taskObj.next_sibling_id, (err_type, err_desc, changesToImplementObj) => {
                                                                console.log('changesToImplementObj: ', changesToImplementObj);
                                                                if (!err_type) {
                                                                    db._manipulateNextAndPrev.checkIfTaskHasSiblingsAndUpdateSiblingRelations_add(pUserId, taskObj, db.updateTaskPreviousSibling, db.updateTaskNextSibling, (err_type, err_desc, changesToImplementAfterPuttingTaskInNewPos) => {
                                                                        let changesToImplementObj = {...changesToImplementAfterRemovingTaskFromOldPos, ...changesToImplementAfterPuttingTaskInNewPos};
                                                                        changesToImplementObj[taskObj.id] = {
...
db.giveOwnerCanGivePermissionsPermission = (pUserId, pTaskId, callback) => {
    // Gives permission to owner so he can give permissions
    // Usually Used when task is first created
    // Returns true if everything went alright
    // false if pUserId does not match pTaskId's creator_user_id 
    let datetime = moment.utc().format('YYYY-MM-DD HH:mm:ss');
    let query_giveOwnerCanGivePermissionsPermission = `INSERT INTO task_permissions(user_id,task_id,permission_type,creator_user_id,date_created) VALUES ("${pUserId}", "${pTaskId}","can_give_permissions", "${pUserId}", "${datetime}")`;
    db.getTask(pUserId, pTaskId, (err_type, err_desc, taskObj)=>{

        if(!err_type){
            // Check if pUserId matches the task creator id
            if(taskObj.creator_user_id == pUserId){
                
                connection.query(query_giveOwnerCanGivePermissionsPermission, (err, result, fields) => {
                    if (!err){
                        callback('','', true);
                    } else {
...
db._permissionsList = ["no_permission", "read_only", "read_and_edit", "can_give_permissions"];
