...
db.addHomePageVisit = (pArrayOfHomePageVisits, callback) => {
    if (connection) {
        if (pArrayOfHomePageVisits.length > 0 && typeof(pArrayOfHomePageVisits) == 'object'){
            let query_addHomePageVisit = `INSERT INTO 
            home_page_visits(ip_address,country,city,visit_date)
            VALUES `;

            query_addHomePageVisit = query_addHomePageVisit + pArrayOfHomePageVisits[0];
            ...
            for (let i = 1; i < pArrayOfHomePageVisits.length; i++ ){
                ...
                let city = HPvisitsObj.country;
                let visit_date = HPvisitsObj.visit_date;

                query_addHomePageVisit = query_addHomePageVisit + ',' + pArrayOfHomePageVisits[i];
            }

            query_addHomePageVisit = query_addHomePageVisit + ";";
            console.log('query_addHomePageVisit: ', query_addHomePageVisit);
